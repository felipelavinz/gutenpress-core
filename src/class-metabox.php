...
abstract class Metabox{
	...
	final public function content_callback(){
		...
		$args = $this->get_args();
		if ( isset($args['context']) && $args['context'] == 'side' ) {
			$form->set_view( 'GutenPress\Forms\View\WP_Side' );
		}
		...
		foreach ( $fields as &$field ) {
			 ...
			 * @param GutenPress\Forms\Element_Interface $field The field instance (passed by reference)
			 * @param GutenPress\Forms\Element\Form $form The form instance (passed by reference)
			 * @param GutenPress\Metabox $this The metabox instance (passed by reference)
			 */
			do_action_ref_array('gutenpress_metabox_field', array(&$field, &$form, &$this) );
			$form->append_node_child( $field );
		}

		// add a nonce for security...
		$nonce = new Forms\Element\WP_Nonce;
		$nonce->set_action( $this->get_id() .'_metabox' )->set_name( $this->get_id() .'_nonce' );
		$form->append_node_child( $nonce );
	 ...
	 */
	private function check_permissions( $postid ){
